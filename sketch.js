...
function draw() {
	...
	for (let i of coins) {
		...
		for (let j of balls) {
			if (
				Math.sqrt(Math.pow(i.split('/')[0] - j.split('/')[0], 2)) <=
					(PROPERTIES.ball_size + PROPERTIES.coin_size) / 2 &&
				Math.sqrt(Math.pow(i.split('/')[1] - j.split('/')[1], 2)) <= (PROPERTIES.ball_size + PROPERTIES.coin_size) / 2
			) {
...
function mousePressed() {
	...
	if (mouseX >= 0 && mouseX <= 167 && mouseY >= 0 && mouseY <= 75) {
		nav_status.balls = true;
		nav_status.coins = false;
		nav_status.lab = false;
	} else if (mouseX >= 167 && mouseX <= 334 && mouseY >= 0 && mouseY <= 75) {
		nav_status.coins = true;
		nav_status.balls = false;
		nav_status.lab = false;
	...
	// BUY SLOTS
		...
		if (nav_status.balls && cash >= UPGRADES.balls[0].price && UPGRADES.balls[0].level < UPGRADES.balls[0].max_level) {
			...
			UPGRADES.coins[0].level < UPGRADES.coins[0].max_level
		) {
			cash -= UPGRADES.coins[0].price;
		...
		if (nav_status.balls && cash >= UPGRADES.balls[1].price && UPGRADES.balls[1].level < UPGRADES.balls[1].max_level) {
			...
			UPGRADES.coins[1].level < UPGRADES.coins[1].max_level
		) {
			cash -= UPGRADES.coins[1].price;
		...
		if (nav_status.balls && cash >= UPGRADES.balls[2].price && UPGRADES.balls[2].level < UPGRADES.balls[2].max_level) {
			...
			UPGRADES.coins[2].level < UPGRADES.coins[2].max_level
		) {
			cash -= UPGRADES.coins[2].price;
